...
def AttackXPUF_by_TRN_PCA(PUFNumber=4, PUFLength=64, Noise=0.01, DataSize=80 * 1000, filename="", num_sample=40 * 1000,
                      train_ratio=0.8, batch_size=10 * 1000, method="Ett", lr=80e-4, rank=[1,1,1,1,1], n_outputs=(1,),
                      epochs=1000, dropout = 0.0 ,save=True,save_model = False,threshhold = 0.97,l2_rate = 0.0001,pca_components=None,model_seed=42,load_seed = 42,data_seed = 42):
    ...
    X_train, Y_train, X_val, Y_val ,_ ,_= loadData_with_PCA_float32(filename, num_sample, train_ratio, phi_length,pca_components=pca_components)
    ...
    if method == "LR":
        log_filename = f"./Save/C2D2/Log/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_epc{epochs}_l2={l2_rate}_{data_seed}_{load_seed}_{model_seed}.txt"
        figure_acc_path = f"./Save/C2D2/Figures/Acc/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_epc{epochs}_l2={l2_rate}_{data_seed}_{load_seed}_{model_seed}_"
        figure_loss_path = f"./Save/C2D2/Figures/Loss/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_epc{epochs}_l2={l2_rate}_{data_seed}_{load_seed}_{model_seed}_"
        loss_acc_path = f"./Save/C2D2/Loss_Acc/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_epc{epochs}_l2={l2_rate}_"
    elif method == "Ett":
        log_filename = f"./Save/{method}_PCA/Log/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}.txt"
        figure_acc_path = f"./Save/{method}_PCA/Figures/Acc/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}_"
        figure_loss_path = f"./Save/{method}_PCA/Figures/Loss/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}_"
        loss_acc_path = f"./Save/{method}_PCA/Loss_Acc/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}_"
    ...
    with open(log_filename, "w") as f:
        for epoch in range(epochs):
            ...
            # X_train = X_train[idx]        #this can save a half of memory
            # Y_train = Y_train[idx]
            ...
            for start_idx in range(0, train_size, batch_size):
                ...
                batch_x = X_train[idx[start_idx:end_idx]]            #this can save a half of memory
            ...
            if save_model == True and epoch > 300 and acc_val > best_val_acc and acc_val > threshhold:
                ModelName = f"./Save/{method}_PCA/Model/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}"
...
if __name__ == "__main__":
    ...
    DataSize = 20500 * 1000
    num_sample = int(16000 * 1000 / train_ratio)
    # # # 6-xor search
    data_seeds = [42]
    load_seeds = [47]
    model_seeds = [70]
                        ...
                        for batch_size in batch_sizes:
                            for l2_rate in l2_rates:
                                for dropout in dropouts:
                                    ...
                                    # TT_rank = [1, 1, 1, 1, r, 1, 1, 1, 1]
                                    TT_rank = [1, 1, r//2, r, r, r//2, 1, 1]
                                    TT_rank = [1, 1, 1, r//2, r, r // 2, 1, 1]
                                    # TT_rank = [1, max(int(r/4),1), int(r/2), r, r, int(r/2), max(int(r/4),1), 1]
                                    # TT_rank = [1, int(r / 4), r, r, r, r, int(r / 4), 1]
                                    ...
                                    exist = False
                                    for fname in os.listdir(f"./Save/C2D2/Log/"):
                                        if fname.startswith(
                                                f"{PUFLength}_{PUFNumber}_{int(num_sample * train_ratio) / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}"):
                                    ...
                                    AttackXPUF_by_TRN_PCA(PUFNumber=PUFNumber, PUFLength=PUFLength, Noise=Noise,
                                                          DataSize=DataSize,
                                                          ...
                                                          pca_components=None, load_seed=load_seed,
