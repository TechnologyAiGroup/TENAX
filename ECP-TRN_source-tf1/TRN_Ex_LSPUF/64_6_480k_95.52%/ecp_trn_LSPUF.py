...
from TRL_LSPUF import ecp_trn_LSPUF
...
df1 = pd.read_csv('./dataset/64_6_2000k_0.01_42_LSPUF.csv', header=None, chunksize=nSamples)
df1 = df1.get_chunk()
X = df1.iloc[:nSamples, :-1]
Y = df1.iloc[:nSamples, [-1]]
...
def main(rank=None, file=None, f_error=None, f_acc=None):
    with tf.Session() as sess:
        ...
        y1 = tf.placeholder(tf.float32, shape=[None, phi_length, phi_length, phi_length, phi_length, phi_length, phi_length])
        x = tf.placeholder(tf.float32, shape=[None, phi_length * n_xor])

        # the correct answer y_
        y_ = tf.placeholder(tf.float32, [None, 1])

        # out = ttrl(x, rank, 1)
        out = ecp_trn_LSPUF(x, y1, rank, 1,phi_length,n_xor)
        ...
        train_step = tf.train.AdamOptimizer(80e-4).minimize(cross_entropy)
