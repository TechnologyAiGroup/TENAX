...
def ecp_trn_LSPUF(x,y1, rank, n_outputs,phi_length,n_xor):
	...
	x= tf.reshape(x, [-1, phi_length * n_xor])

	cout=tf.zeros([n_outputs],tf.float32)
	for j in range(0,len(rank1_tnsrs)):
		tout=tf.multiply(tf.scalar_mul(1,tf.matmul(x[:,phi_length*0:phi_length*1],tf.reshape(rank1_tnsrs[j][0], [-1,1]))),tf.scalar_mul(1,tf.matmul(x[:,phi_length*1:phi_length*2],tf.reshape(rank1_tnsrs[j][1], [-1,1]))))
		for k in range(2,len(rank1_tnsrs[j])-1):
			tout=tf.multiply(tout,tf.scalar_mul(1,tf.matmul(x[:,phi_length*k:phi_length*(k+1)],tf.reshape(rank1_tnsrs[j][k], [-1,1]))))
