...
def AttackXPUF_LR(PUFNumber=4, PUFLength=64, X=None, Y=None,
                  train_ratio=0.95, batch_size=10 * 1000, method="LR", lr=80e-4, rank=None,
                  epochs=1000, l2_rate=0, model_seed=42, load_seed=42):
    print(
        f"PUFNumber: {PUFNumber} ,PUFLength: {PUFLength} , num_sample:{X.shape[0]}")
    print(
        f"method: {method}, lr: {lr}, batch_size: {batch_size}, epochs: {epochs} , l2_rate : {l2_rate},load_seed :{load_seed}, model_seed :{model_seed}")
...
def train_and_load_model_u(C, R, model_d, batch_size, epochs, Ku, n, cnt, f,
    ...
    model_path = f"{save_dir}/FPGA_LR_Model_1_4_{cnt}_{load_seed}_{model_seed}_up"
    best_val_acc = 0
    if not Saved:
...
def train_and_load_model_d(C, R, model_u,  batch_size, epochs, Kd, n, cnt, f,
    ...
    model_path = f"{save_dir}/FPGA_LR_Model_1_4_{cnt}_{load_seed}_{model_seed}_down"
    best_val_acc = 0
    if not Saved:
        model_d, best_val_acc = AttackXPUF_LR(
            PUFNumber=Kd,PUFLength=n + 1,X=C_d,Y=R,
            train_ratio=0.95,batch_size=batch_size,
...
def AttackMXPUF_PCA(Ku=1, Kd=4, PUFLength=64, filename="", num_sample=40 * 1000,
                    save=True):

    data = np.loadtxt(filename, delimiter=",")
    assert data.shape[0] >= num_sample + 2000, "num_sample Error"
    SetSeed(43)
    np.random.shuffle(data)

    C = data[:num_sample, :PUFLength]
    R = data[:num_sample, -1].reshape(num_sample, 1)
    C_test = data[num_sample:num_sample + 2000, :PUFLength]
    R_test = data[num_sample:num_sample + 2000, -1].reshape(2000, 1)
    ...
    for lr in lrs:
        ...
        batch_size = int(min(600 * 1000, 800 * 1000))
        log_filename = f"./Save/MXPUF/LR/Log/FPGA_{PUFLength}_{Ku}_{Kd}_{int(num_sample * 0.95) / 1000:.0f}k_{batch_size}_{lr:.4f}_epc{epochs}_{load_seed}_{model_seed}_firstmodel.txt"


        saved = False
        if not saved:
            model_d, best_val_acc = AttackXPUF_LR(PUFNumber=Kd, PUFLength=n + 1,X=C_d, Y=R,train_ratio=0.95,batch_size=batch_size,method="LR",lr=lr, epochs=epochs,model_seed=model_seed,load_seed=load_seed)
            ...
            if best_val_acc < 0.64:
                ...
                model_d.save("Save/MXPUF/Models/FPGA_LR_Model_1_4")




        acc_test = 0
        log_filename = f"./Save/MXPUF/LR/Log/FPGA_{PUFLength}_{Ku}_{Kd}_{int(num_sample * 0.95) / 1000:.0f}k_{batch_size}_{lr:.4f}_epc{epochs}_{load_seed}_{model_seed}.txt"

        with open(log_filename, "w") as f:

            # 载入初始model_d（只第一次需要）
            model_d = tf.keras.models.load_model("Save/MXPUF/Models/FPGA_LR_Model_1_4")
...
if __name__ == "__main__":
    ...
    filename = f"./Datasets/iPUF_128_1_4_CRPs.csv"
    num_sample = int(600 * 1000 / 0.95)
    save = True

    AttackMXPUF_PCA(Ku=Ku, Kd=Kd, PUFLength=PUFLength, filename=filename,
