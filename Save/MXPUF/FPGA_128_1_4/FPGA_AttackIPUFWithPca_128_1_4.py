...
def AttackXPUF_PCA(PUFNumber=4, PUFLength=64, X=None, Y=None,
                   train_ratio=0.8
                   , batch_size=10 * 1000, method="Ett", lr=80e-4, rank=None, n_outputs=(1,),
...
def train_and_load_model_u(C, R, model_d, mu_d, coeff_d, batch_size, epochs, Ku, n, cnt, f,
    ...
    model_path = f"{save_dir}/FPGA_Model_1_4_{cnt}_{load_seed}_{model_seed}_up"
    best_val_acc = 0
    if not Saved:
...
def train_and_load_model_d(C, R, model_u, mu_u, coeff_u, batch_size, epochs, Kd, n, cnt, f,
    ...
    model_path = f"{save_dir}/FPGA_Model_1_4_{cnt}_{load_seed}_{model_seed}_down"
    best_val_acc = 0
    if not Saved:
                ...
                for lr in lrs:
                    for l2_rate in l2_rates:
                        for dropout in dropouts:
                            ...
                            batch_size = int(min(200 * 1000, 800 * 1000))
                            log_filename = f"./Save/MXPUF/Ett_PCA/Log/FPGA_{PUFLength}_{Ku}_{Kd}_{int(num_sample * 0.95) / 1000:.0f}k_{batch_size}_{lr:.4f}_{TT_rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{load_seed}_{model_seed}_firstmodel.txt"
                            ...
                            if not saved:
                                ...
                                if best_val_acc < 0.57:
                                    continue
                                else:
                                    model_d.save("Save/MXPUF/Models/FPGA_Model_1_4")
                                    np.savez("Save/MXPUF/Models/FPGA_mu_coeff_1_4.npz", mu_d=mu_d, coeff_d=coeff_d)


                            acc_test = 0
                            log_filename = f"./Save/MXPUF/Ett_PCA/Log/FPGA_{PUFLength}_{Ku}_{Kd}_{int(num_sample * 0.95) / 1000:.0f}k_{batch_size}_{lr:.4f}_{TT_rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{load_seed}_{model_seed}.txt"

                            with open(log_filename, "w") as f:

                                # 载入初始model_d和PCA参数（只第一次需要）
                                model_d = tf.keras.models.load_model("Save/MXPUF/Models/FPGA_Model_1_4")
                                pca_data = np.load("Save/MXPUF/Models/FPGA_mu_coeff_1_4.npz")
                                mu_d = pca_data['mu_d']
                                coeff_d = pca_data['coeff_d']
...
if __name__ == "__main__":
    ...
    num_sample = int(200 * 1000 / 0.95)
    save = True

    AttackMXPUF_PCA(Ku=Ku, Kd=Kd, PUFLength=PUFLength, filename=filename,num_sample=num_sample,save=save)