...
def train_and_load_model_u(C, R, model_d, batch_size, epochs, Ku, n, cnt, f,
    ...
    model_path = f"{save_dir}/FPGA_LR_Model_1_5_{cnt}_{load_seed}_{model_seed}_up"
    best_val_acc = 0
    if not Saved:
...
def train_and_load_model_d(C, R, model_u,  batch_size, epochs, Kd, n, cnt, f,
    ...
    model_path = f"{save_dir}/FPGA_LR_Model_1_5_{cnt}_{load_seed}_{model_seed}_down"
    best_val_acc = 0
    if not Saved:
...
def AttackMXPUF_PCA(Ku=1, Kd=4, PUFLength=64, filename="", num_sample=40 * 1000,
    ...
    for lr in lrs:
        ...
        batch_size = int(min(500 * 1000, 800 * 1000))
        ...
        if not saved:
            ...
            if best_val_acc < 0.64:
                ...
                model_d.save("Save/MXPUF/Models/FPGA_LR_Model_1_5")
        ...
        with open(log_filename, "w") as f:
            ...
            # 载入初始model_d（只第一次需要）
            model_d = tf.keras.models.load_model("Save/MXPUF/Models/FPGA_LR_Model_1_5")
...
if __name__ == "__main__":
    ...
    filename = f"./Datasets/iPUF_64_1_5_CRPs.csv"
    num_sample = int(500 * 1000 / 0.95)
    save = True
