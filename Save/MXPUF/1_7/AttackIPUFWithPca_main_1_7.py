...
def AttackXPUF_PCA(PUFNumber=4, PUFLength=64, X=None, Y=None,
                   train_ratio=0.8
                   , batch_size=10 * 1000, method="Ett", lr=60e-4, rank=None, n_outputs=(1,),
...
def train_and_load_model_u(C, R, model_d, mu_d, coeff_d, batch_size, epochs, Ku, n, cnt, f,
    ...
    model_path = f"{save_dir}/Model_1_7_{cnt}_{load_seed}_{model_seed}_up"
    best_val_acc = 0
    if not Saved:
...
def train_and_load_model_d(C, R, model_u, mu_u, coeff_u, batch_size, epochs, Kd, n, cnt, f,
    ...
    model_path = f"{save_dir}/Model_1_7_{cnt}_{load_seed}_{model_seed}_down"
    best_val_acc = 0
    if not Saved:
                        ...
                        for dropout in dropouts:
                            ...
                            if not saved:
                                ...
                                if best_val_acc < 0.63:
                                    ...
                                    model_d.save("Save/MXPUF/Models/Model_1_7")
                                    np.savez("Save/MXPUF/Models/mu_coeff_1_7.npz", mu_d=mu_d, coeff_d=coeff_d)
                            ...
                            with open(log_filename, "w") as f:
                                ...
                                model_d = tf.keras.models.load_model("Save/MXPUF/Models/Model_1_7")
                                pca_data = np.load("Save/MXPUF/Models/mu_coeff_1_7.npz")
                                mu_d = pca_data['mu_d']
                                coeff_d = pca_data['coeff_d']
                                ...
                                lr_d = 60e-4
                                TT_rank_d = [1, 1, 1, 1, 1, 1, 1, 1]
                                dropout_d = 0
                                l2_rate_d = 1e-4
                                load_seed_d = load_seed
                                model_seed_d = model_seed
                                for num in range(50):
                                    if num >= 1 :
                                        np.random.seed()
