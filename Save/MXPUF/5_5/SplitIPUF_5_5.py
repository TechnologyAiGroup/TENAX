...
def train_and_load_model_u(C, R, model_d, batch_size, epochs, Ku, n, cnt, f,
    ...
    model_path = f"{save_dir}/LR_Model_5_5_{cnt}_{load_seed}_{model_seed}_up"
    best_val_acc = 0
    if not Saved:
...
def train_and_load_model_d(C, R, model_u,  batch_size, epochs, Kd, n, cnt, f,
    ...
    model_path = f"{save_dir}/LR_Model_5_5_{cnt}_{load_seed}_{model_seed}_down"
    best_val_acc = 0
    if not Saved:
    ...
    for lr in lrs:
        ...
        if not saved:
            ...
            if best_val_acc < 0.64:
                ...
                model_d.save("Save/MXPUF/Models/LR_Model_5_5")
        ...
        with open(log_filename, "w") as f:
            ...
            # 载入初始model_d（只第一次需要）
            model_d = tf.keras.models.load_model("Save/MXPUF/Models/LR_Model_5_5")
