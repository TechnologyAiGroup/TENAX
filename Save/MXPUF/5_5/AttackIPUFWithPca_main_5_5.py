...
def train_and_load_model_u(C, R, model_d, mu_d, coeff_d, batch_size, epochs, Ku, n, cnt, f,
    ...
    model_path = f"{save_dir}/Model_5_5_{cnt}_{load_seed}_{model_seed}_up"
    best_val_acc = 0
    if not Saved:
...
def train_and_load_model_d(C, R, model_u, mu_u, coeff_u, batch_size, epochs, Kd, n, cnt, f,
    ...
    model_path = f"{save_dir}/Model_5_5_{cnt}_{load_seed}_{model_seed}_down"
    best_val_acc = 0
    if not Saved:
                ...
                for lr in lrs:
                    for l2_rate in l2_rates:
                        for dropout in dropouts:
                            ...
                            batch_size = int(min(300 * 1000, 800 * 1000))
                            ...
                            if not saved:
                                ...
                                if best_val_acc < 0.63:
                                    ...
                                    model_d.save("Save/MXPUF/Models/Model_5_5")
                                    np.savez("Save/MXPUF/Models/mu_coeff_5_5.npz", mu_d=mu_d, coeff_d=coeff_d)
                            ...
                            with open(log_filename, "w") as f:
                                ...
                                model_d = tf.keras.models.load_model("Save/MXPUF/Models/Model_5_5")
                                pca_data = np.load("Save/MXPUF/Models/mu_coeff_5_5.npz")
                                mu_d = pca_data['mu_d']
                                coeff_d = pca_data['coeff_d']
