...
def AttackXPUF_by_TRN(PUFNumber=4, PUFLength=64, Noise=0.01, DataSize=80 * 1000, filename="", num_sample=40 * 1000,
                      train_ratio=0.8, batch_size=10 * 1000, method="Ett", lr=80e-4, rank=[1,1,1,1,1], n_outputs=(1,),
                      epochs=1000, dropout = 0.0 ,save=True,save_model = False,threshhold = 0.97,l2_rate = 0.0001,model_seed=42,load_seed = 42,data_seed = 42):
    ...
    X_train, Y_train, X_val, Y_val = loadData_float32(filename, num_sample, train_ratio, phi_length)
    ...
    if method == "LR":
        log_filename = f"./Save/{method}/Log/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_epc{epochs}_l2={l2_rate}_{data_seed}_{load_seed}_{model_seed}.txt"
        figure_acc_path = f"./Save/{method}/Figures/Acc/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_epc{epochs}_l2={l2_rate}_{data_seed}_{load_seed}_{model_seed}_"
        figure_loss_path = f"./Save/{method}/Figures/Loss/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_epc{epochs}_l2={l2_rate}_{data_seed}_{load_seed}_{model_seed}_"
        loss_acc_path = f"./Save/{method}/Loss_Acc/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_epc{epochs}_l2={l2_rate}_"
    elif method == "Ett":
        log_filename = f"./Save/{method}/Log/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}.txt"
        figure_acc_path = f"./Save/{method}/Figures/Acc/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}_"
        figure_loss_path = f"./Save/{method}/Figures/Loss/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}_"
        loss_acc_path = f"./Save/{method}/Loss_Acc/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}_"
    ...
    with open(log_filename, "w") as f:
        for epoch in range(epochs):
            ...
            if save_model == True and epoch > 300 and acc_val > best_val_acc and acc_val > threshhold:
                ModelName = f"./Save/{method}/Model/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{data_seed}_{load_seed}_{model_seed}"
                        ...
                        for batch_size in batch_sizes:
                            for l2_rate in l2_rates:
                                for dropout in dropouts:
                                    ...
                                    TT_rank = [1, 1, 1, r, r, r, 1, 1, 1]
                                    # TT_rank = [1, 1, 1, 1, r, 1, 1, 1, 1]
                                    TT_rank = [1, 1, r, r, r, r, 1, 1]
                                    ...
                                    exist = False
                                    for fname in os.listdir(f"./Save/{method}/Log/"):
                                        if fname.startswith(
                                    ...
                                    AttackXPUF_by_TRN(PUFNumber=PUFNumber, PUFLength=PUFLength, Noise=Noise,
                                                          DataSize=DataSize,
