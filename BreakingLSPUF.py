...
def AttackLSPUF(PUFNumber=4, PUFLength=64, Noise=0.01, DataSize=80 * 1000, filename="", num_sample=40 * 1000,
                      train_ratio=0.8, batch_size=10 * 1000, method="LR", lr=80e-4,
                      epochs=1000,save=True,model_seed=42,load_seed = 42,data_seed = 42):
    ...
    log_filename = f"./Save/LSPUF/BreakingLSPUF/Log/{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_{Noise}_LSPUF_{batch_size}_{lr:.4f}_epc{epochs}_{data_seed}_{load_seed}_{model_seed}.txt"
...
if __name__ == "__main__":
    ...
    if PUFLength == 64:
        ...
        if PUFNumber == 4:
            ...
            num_sample = int(300 * 1000 / 0.95)
        elif PUFNumber == 6:
            num_sample = int(1000 * 1000 / 0.95)
        ...
        for model_seed in model_seeds:
            for lr in lrs:
                for batch_size in batch_sizes:
                    ...
                    exist = False
                    for fname in os.listdir(f"./Save/LSPUF/BreakingLSPUF/Log/"):
                        if fname.startswith(
                                f"{PUFLength}_{PUFNumber}_{int(num_sample * train_ratio) / 1000:.0f}k_{Noise}_LSPUF_{batch_size}_{lr:.4f}_epc{epochs}_{data_seed}_{load_seed}_{model_seed}"):
                    ...
                    print(
                        f"method: {method}, lr: {lr}, batch_size: {batch_size}, epochs: {epochs} , data_seed :{data_seed},load_seed :{load_seed}, model_seed :{model_seed}")

                    AttackLSPUF(PUFNumber=PUFNumber, PUFLength=PUFLength, Noise=Noise,
                                DataSize=DataSize,
                                filename=filename, num_sample=num_sample,
                                train_ratio=train_ratio,
                                batch_size=batch_size, method=method, lr=lr, epochs=epochs, save=True,
                                load_seed=load_seed,
