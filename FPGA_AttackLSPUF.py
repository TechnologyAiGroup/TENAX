...
def AttackLSPUF_by_TRN(PUFNumber=4, PUFLength=64, filename="", num_sample=40 * 1000,
                      train_ratio=0.8, batch_size=10 * 1000, method="Ett", lr=80e-4, rank=[1,1,1,1,1], n_outputs=(1,),
                      epochs=1000, dropout = 0.0 ,save=True,l2_rate = 0.0001,model_seed=42,load_seed = 42):
    ...
    if method == "Ett":
        log_filename = f"./Save/LSPUF/{method}/Log/FPGA_{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_LSPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{load_seed}_{model_seed}.txt"
...
if __name__ == "__main__":
    ...
    num_sample = int(120 * 1000 / train_ratio)
    # # # 6-xor search
    load_seeds = [6]
    model_seeds = [70]
    l2_rates = [0]
    lrs = [60e-4,80e-4]
    batch_sizes = [120 * 1000]
    rs = [1]*50
    dropouts = [0]
                    ...
                    for batch_size in batch_sizes:
                        for l2_rate in l2_rates:
                            for dropout in dropouts:
                                ...
                                AttackLSPUF_by_TRN(PUFNumber=PUFNumber, PUFLength=PUFLength, num_sample=num_sample,
                                                   train_ratio=train_ratio,filename=proceed_filename,
