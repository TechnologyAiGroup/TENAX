...
def AttackXPUF_by_TRN_PCA(PUFNumber=4, PUFLength=64, filename="", num_sample=40 * 1000,
                      train_ratio=0.8, batch_size=10 * 1000, method="Ett", lr=80e-4, rank=[1,1,1,1,1], n_outputs=(1,),
                      epochs=1000, dropout = 0.0 ,save=True,l2_rate = 0.0001,pca_components=None,model_seed=42,load_seed = 42):
    ...
    if method == "LR":
        log_filename = f"./Save/C2D2/Log/FPGA_{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_XORAPUF_{batch_size}_{lr:.4f}_epc{epochs}_l2={l2_rate}_{load_seed}_{model_seed}.txt"
    elif method == "Ett":
        log_filename = f"./Save/{method}_PCA/Log/FPGA_{PUFLength}_{PUFNumber}_{train_size / 1000:.0f}k_XORAPUF_{batch_size}_{lr:.4f}_{rank}_epc{epochs}_l2={l2_rate}_dp={dropout}_{load_seed}_{model_seed}.txt"
                    ...
                    for batch_size in batch_sizes:
                        for l2_rate in l2_rates:
                            for dropout in dropouts:
                                ...
                                AttackXPUF_by_TRN_PCA(PUFNumber=PUFNumber, PUFLength=PUFLength,
